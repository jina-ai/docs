...
.. note:: Currently we do not support native Python on Windows. If you are a Windows user, one workaround is to `run Jina on Windows Subsystem for Linux <../on-wsl>`_ or `run Jina in a Docker container <via-docker.md>`_.
